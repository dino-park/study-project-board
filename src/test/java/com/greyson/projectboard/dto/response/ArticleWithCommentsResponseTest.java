...
class ArticleWithCommentsResponseTest {
    ...
    @DisplayName("자식 댓글이 없는 게시글+댓글 dto를 api응답으로 변환 시, 댓글을 시간 내림차순 + ID 오름차순으로 정리")
    @Test
    ...
    @DisplayName("게시글+댓글 dto를 api응답으로 변환 시, 댓글 부모 자식 관계를 각각의 규칙으로 정렬하여 정리")
    @Test
    void givenArticleWithCommentsDto_whenMapping_thenOrganizesParentAndChildCommentsWithCertainOrder() {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Set<ArticleCommentDto> articleCommentDtos = Set.of(
                createArticleCommentDto(1L, null, now),
    ...
    @DisplayName("게시글+댓글 dto를 api응답으로 변환 시, 부모 자식 관계 깊이(depth)는 제한이 없음")
    @Test
